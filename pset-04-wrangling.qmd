...
population <- population |> 
  row_to_names(1) |> ## Use janitor row to names function
...
cases_raw |>
  ...
  mutate(cases = as.numeric(cases), date = as.Date(date)) -> cases
...
cases |> 
  ...
  group_split(region_name) |>
  map(~ggplot(.x, aes(x=date, y=per_100000, colour = state)) + 
    geom_line() + 
    labs(title = unique(.x$region_name))) 
```
...
deaths |>
  ...
  arrange(desc(deaths)) |>
  head(10) |>
  ggplot(aes(x=reorder(state,-deaths), y=deaths, color=state)) +
    geom_col()
